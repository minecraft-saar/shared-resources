...
(:method (build-stairs ?x ?y ?z ?width ?depth ?height ?dir)
    build-wall
    ()
    ((build-stairs-wall ?x ?y ?z ?width ?depth ?height ?dir))
)
...
(:method (build-stairs-wall ?x ?y ?z ?width ?depth ?height ?dir)
    height-one
    ((call equal ?height 1))
    ((build-row ?x ?y ?z ?width ?dir))

    east
    ((not (call equal ?height 1)) (call equal ?dir 1))
    (
        (build-railing ?x ?y ?z ?width ?height ?dir)
        (build-stairs-wall ?x ?y (call + ?z 1) ?width (call - ?depth 1) (call - ?height 1) ?dir)
    )

    west
    ((not (call equal ?height 1)) (call equal ?dir 2))
    (
        (build-railing ?x ?y ?z ?width ?height ?dir)
        (build-stairs-wall ?x ?y (call + ?z 1) ?width (call - ?depth 1) (call - ?height 1) ?dir)
    )
    ...
    (
        (build-railing ?x ?y ?z ?width ?height ?dir)
        (build-stairs-wall (call + ?x 1) ?y ?z ?width (call - ?depth 1) (call - ?height 1) ?dir)
    )
...
(:method (build-bridge ?x ?y ?z ?width ?length ?height)
        ()
        (
            (build-stairs ?x ?y ?z ?width ?height ?height 4)
            (build-floor ?x (call - (call + ?y ?height) 1) (call -(call + ?z ?height) 1) ?length ?width 4)
            (build-stairs ?x ?y (call - (call + (call + ?z ?length) ?height) 1) ?width ?height ?height 3)
            (build-railing ?x (call + ?y ?height) (call - (call + (call + ?z ?length) ?height) 1) ?length 2 3)
            (build-railing (call - (call + ?x ?width ) 1) (call + ?y ?height) (call + ?z ?height) ?length 2 4)
        )
)   
